...
void MapMomentsToServoAngles(double Mx, double My, double Mz, uint8_t throttle) {
    double roll_rudder, pitch_rudder, yaw_rudder;
    ...
    pitch_rudder = constrain(pitch_rudder, -45.0, 45.0);
    yaw_rudder = constrain(yaw_rudder, -45.0, 45.0);

    // Set servo angles with Saturation
    ServoControl_SetAngle((int8_t)(-pitch_rudder+yaw_rudder), (int8_t)(-roll_rudder+yaw_rudder), 
                        (int8_t)(pitch_rudder+yaw_rudder), (int8_t)(roll_rudder+yaw_rudder));
}
