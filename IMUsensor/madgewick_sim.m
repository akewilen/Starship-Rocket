...
for k = 2:N
    q_next = madgewick_filter(mag_corr(k,:), acc_corr(k,:), gyro_corr(k,:), gyro_error,  q0, dt);
    q_mat(:, k) = q_next;
    q0 = q_next;
...
%% Madgewick vs Accelerometer estimation of roll, pitch and yaw
figure
subplot(3,1,1);
plot(sim_time, euler_angles(1,:))
hold on
plot(sim_time,euler_angles_acc(1,:))
...
hold on
plot(sim_time,euler_angles_acc(2,:))
xlabel("Time [s]")
ylabel("Pitch [d]")
